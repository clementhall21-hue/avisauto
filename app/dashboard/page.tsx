...
export default function DashboardPage() {
  ...
  return (
    <div className="min-w-0">
      {/* Header */}
      <div className="mb-6">
        <h2 className="text-xl md:text-2xl font-bold tracking-tight text-[#e8eaf6]">
          Bonjour{userName ? `, ${userName}` : ''} 👋
        </h2>
        <p className="text-sm text-[#8892b0] mt-1 truncate">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
        {loading
          ? [1,2,3,4].map((i) => (
              <div key={i} className="bg-[rgba(255,255,255,0.03)] border border-[rgba(255,255,255,0.07)] rounded-xl p-4">
          ...
          : stats.map((stat, i) => (
              <div
                key={i}
                className="bg-[rgba(255,255,255,0.03)] border border-[rgba(255,255,255,0.07)] rounded-xl p-4 min-w-0"
                style={{ borderTopColor: stat.color, borderTopWidth: 2 }}
              >
                <div className="flex items-center justify-between mb-2 gap-1">
                  <div className="text-[0.65rem] text-[#8892b0] font-medium uppercase tracking-wider truncate">{stat.label}</div>
                  <stat.icon size={13} style={{ color: stat.iconColor }} className="flex-shrink-0" />
                </div>
                <div className="text-[1.5rem] md:text-[1.8rem] font-bold tracking-tight text-[#e8eaf6]">{stat.value}</div>
              </div>
            ))}
      </div>
      ...
      <div className="grid md:grid-cols-3 gap-4">
        {/* Recent activity */}
        <div className="md:col-span-2 bg-[rgba(255,255,255,0.03)] border border-[rgba(255,255,255,0.07)] rounded-xl p-4 min-w-0">
            ...
            <div className="space-y-1">
              {recent.map((r) => (
                <Link
                  ...
                  className="flex items-start gap-2 p-3 rounded-lg hover:bg-[rgba(255,255,255,0.03)] transition-colors group"
                >
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-[#8892b0] truncate">{r.review_text}</p>
                  </div>
                  <span className={`text-[0.6rem] font-semibold px-1.5 py-0.5 rounded flex-shrink-0 whitespace-nowrap ${
                    r.status === 'published' ? 'bg-[rgba(52,211,153,0.12)] text-[#34d399]' :
                    r.status === 'scheduled' ? 'bg-[rgba(99,102,241,0.12)] text-[#818cf8]' :
        ...
        <div className="space-y-3">
          <div className="bg-[rgba(255,255,255,0.03)] border border-[rgba(255,255,255,0.07)] rounded-xl p-4">
            <h3 className="font-semibold text-[#e8eaf6] mb-3 text-sm">Actions rapides</h3>
            <div className="space-y-2">
              <Link
                href="/dashboard/reviews"
                className="flex items-center gap-3 p-3 rounded-lg bg-[rgba(99,102,241,0.08)] border border-[rgba(99,102,241,0.2)] hover:bg-[rgba(99,102,241,0.15)] transition-colors"
              >
                <BarChart2 size={16} className="text-[#818cf8] flex-shrink-0" />
                <div className="min-w-0">
              ...
              >
                <Settings size={16} className="text-[#8892b0] flex-shrink-0" />
          ...
          {pending > 0 && (
            <div className="bg-[rgba(245,158,11,0.08)] border border-[rgba(245,158,11,0.2)] rounded-xl p-4">
              ...
              <p className="text-xs text-[#8892b0] mb-2">Des avis n&apos;ont pas encore reçu de réponse.</p>
