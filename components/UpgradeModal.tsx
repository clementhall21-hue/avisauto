      ...
      <Dialog.Portal>
        ...
        <Dialog.Content className="fixed left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 z-[201] w-[calc(100vw-32px)] max-w-[440px] bg-[#111827] border border-[rgba(99,102,241,0.3)] rounded-2xl p-6 md:p-8 shadow-[0_24px_80px_rgba(0,0,0,0.6)] data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 overflow-y-auto max-h-[90vh]">
          ...
          <div className="text-center mb-6">
            <div className="inline-flex items-center justify-center w-12 h-12 rounded-full bg-[rgba(99,102,241,0.15)] border border-[rgba(99,102,241,0.3)] text-2xl mb-4">
              ✨
            </div>
            <Dialog.Title className="text-xl font-bold text-[#e8eaf6] mb-2">
              Fonctionnalité Pro
            </Dialog.Title>
            <Dialog.Description className="text-[#8892b0] text-sm leading-relaxed">
              <span className="text-[#e8eaf6] font-medium">{feature}</span> est disponible avec le plan Pro.
            </Dialog.Description>
          </div>

          <ul className="space-y-2 mb-7">
            {PRO_BENEFITS.map((benefit) => (
              <li key={benefit} className="flex items-center gap-2.5 text-sm text-[#8892b0]">
          ...
          <p className="text-center text-xs text-[#8892b0] mt-3">14 jours d&apos;essai gratuit inclus</p>
        </Dialog.Content>
      </Dialog.Portal>
    </Dialog.Root>
  )
